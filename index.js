...
function getWinnersByYear(array, finalsCB, winnersCB) {
    let winners = [];
    let year = finalsCB(array);
    let country = winnersCB(array);
...
function getAverageGoals(finalsCB) {
    ...
    let averageGoals = finalsCB.reduce((total, item) => {
       return (total + item[`Home Team Goals`] + item[`Away Team Goals`])
   }, 0)
   return (averageGoals/finalsCB.length).toFixed(2);
}
